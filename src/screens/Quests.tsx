    ...
    <div data-screen-label="Quests" style={{ padding: '56px 18px 116px' }}>
      ...
      <div style={{ ...card, position: 'relative', padding: 18, marginBottom: 18, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          <ProgressBar pct={teamPct} fill={T.accent} track="rgba(255,255,255,0.08)" height={14} />
        </div>
      ...
      <div style={{ ...card, padding: 16, marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 15 }}>
        {dailyGoals.map((g) => (
          <div key={g.label}>
            ...
            <ProgressBar pct={g.pct} fill={g.color} track="rgba(255,255,255,0.08)" />
          </div>
        ))}
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
            {d.joinedChallenges.map((jc) => (
              <div key={jc.challenge.id} style={{ ...card, padding: 16 }}>
                ...
                <ProgressBar pct={jc.pct} fill={jc.challenge.color} track="rgba(255,255,255,0.08)" />
        ...
        return (
          <div key={c.id} style={{ ...card, padding: 16, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 13 }}>
              <div style={{ ...inset, width: 50, height: 50, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: `1px solid ${T.lineSoft}`, paddingTop: 12 }}>
              ...
              <button
                ...
                className="pressable"
                style={{ background: joined ? T.glass : c.color, color: joined ? T.green : T.ink, border: 'none', borderRadius: 14, padding: '9px 18px', fontFamily: T.display, fontWeight: 600, fontSize: 14, cursor: joined ? 'default' : 'pointer', boxShadow: 'none' }}
              >
                {joined ? 'Joined ✓' : 'Join'}
              </button>
            </div>
