...
export async function renderMealStory(canvas: HTMLCanvasElement, opts: StoryOptions) {
  ...
  // Footer: Pip + wordmark.
  const pip = await loadImage('data:image/svg+xml;utf8,' + encodeURIComponent(PIP_SVG))
  const ps = 150
  const fy = H - 190
